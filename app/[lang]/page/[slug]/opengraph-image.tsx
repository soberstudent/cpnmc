...
export const alt = 'Tutorialsmaterial | Blog';
      ...
      <div tw="relative flex w-full h-full flex items-center justify-center">
        ...
        <div tw="absolute flex inset-0">
          <img
            ...
            src={`${process.env.NEXT_PUBLIC_ASSETS_URL}${page.image}`}
            alt={page?.title!!}
          />
          {/* Overlay */}
        ...
        <div tw="flex flex-col text-neutral-50 ">
          {/* Title */}
          <div tw="text-[60px]">{page?.title}</div>
          {/* Description */}
          <div tw="text-2xl max-w-4xl">{page?.description}</div>
          {/* Tags */}
          <div tw="flex mt-6 flex-wrap items-center text-3xl text-neutral-200">
            ...
            <div tw="w-4 h-4 mx-6 rounded-full bg-neutral-300" />
            <div>{getReadingTime(page?.body!!, lang)}</div>
            <div tw="w-4 h-4 mx-6 rounded-full bg-neutral-300" />
            <div>{getRelativeDate(page?.date_created!!, lang)}</div>
          </div>
        </div>
      </div>
    ),
